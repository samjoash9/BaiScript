...
export default function Docs() {
    ...
    return (
        <div className={`min-h-screen transition-colors ${isDarkMode
            ...
            <header
                className={`sticky top-0 z-50 border-b backdrop-blur-sm transition-colors ${isDarkMode
                    ? 'border-gray-800 bg-black/50'
                    : 'border-gray-300/70 bg-gray-200'}`}
                style={{ WebkitAppRegion: 'drag' } as React.CSSProperties}
            >
                <div className="flex items-center justify-between px-6 py-4">
                    {/* Left: Logo + Title - This area is draggable */}
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h1 className={`text-lg font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>BaiScript Documentation</h1>
                            <p className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Multi-stage Compiler Environment</p>
                        </div>
                    </div>

                    {/* Right: Theme Toggle + Back Button - These are explicitly no-drag */}
                    <div className="flex items-center gap-3">
                        {/* Theme Toggle */}
                        <button
